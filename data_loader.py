...
========================================
Cambios v4.4:
* load_produccion ahora acepta Excel nuevo de producción además de CSV.
* Normaliza columnas del nuevo esquema:
    - _match_type -> match_method
    - screenPageViews -> ga4_views
    - activeUsers -> ga4_users
* Si el Excel ya trae ga4_views / ga4_users / match_method, se respeta
  esa información y no se recalcula el matching.
* Mantiene compatibilidad con el flujo anterior basado en Produccion.csv.
"""
...
def _read_excel(fname: str, sheet) -> pd.DataFrame:
    """
    Lee un sheet de Excel. Si ya existe un parquet más reciente que el Excel,
    lo usa directamente. Si no, lee el Excel y guarda parquet.
    """
    src = DATA_DIR / fname
    if not src.exists():
        return pd.DataFrame()
...
# =============================================================================
# LOADERS — Search Console, Producción, Monetización, YouTube
# =============================================================================
...
def _load_produccion_excel_first() -> pd.DataFrame:
    candidatos_excel = [
        "Produccion.xlsx",
        "Producción.xlsx",
        "produccion.xlsx",
...
@st.cache_data(ttl=3600)
def load_produccion():
    df = _load_produccion_excel_first()
    ...
    for col in ["ga4_views", "ga4_users", "userEngagementDuration", "post_author"]:
    ...
    if "permalink" in df.columns and "_prod_path" not in df.columns:
        df["_prod_path"] = df["permalink"].apply(
            lambda u: urlparse(str(u)).path.rstrip("/").lower() if pd.notna(u) else ""
        )
...
def load_produccion_con_metricas() -> pd.DataFrame:
    ...
    ya_viene_match = all(c in prod.columns for c in ["ga4_views", "ga4_users", "match_method"])

    if ya_viene_match:
        result = prod.copy()
    elif _HAS_MATCHING and _match_prod_fn is not None:
