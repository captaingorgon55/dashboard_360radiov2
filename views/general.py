...
prod_con_match = (
    prod_work[prod_work["ga4_views"] > 0]
    if not prod_work.empty and "ga4_views" in prod_work.columns
    else pd.DataFrame()
)
...
if not prod_con_match.empty and "post_author_name" in prod_con_match.columns:
    authors = (
        prod_con_match["post_author_name"]
        .replace("", "Sin autor")
        .fillna("Sin autor")
        .drop_duplicates()
        .tolist()
    )
    auth_list += sorted([a for a in authors if a != ""])

cat_list = ["Todas"]
if not prod_con_match.empty and "categories" in prod_con_match.columns:
    cat_list += (
        prod_con_match["categories"].fillna("")
        .apply(_split_cats).explode().dropna().replace("", pd.NA).dropna()
...
prod = filter_by_date(prod_work, "post_date", start, end) if "post_date" in prod_work.columns else prod_work.copy()
if has_aut and "post_author_name" in prod.columns:
    if sel_aut == "Sin autor":
        prod = prod[prod["post_author_name"].replace("", "Sin autor") == sel_aut].copy()
    else:
...
def _prev_editorial(urls_r_full, prod_r_full, prev_s, prev_e, sel_aut, sel_cat, has_aut, has_cat):
    ...
    if has_aut and "post_author_name" in prod_p.columns:
        if sel_aut == "Sin autor":
            prod_p = prod_p[prod_p["post_author_name"].replace("", "Sin autor") == sel_aut]
        else:
...
if has_editorial:
    ...
    _src_p = _prev_editorial(urls_r, prod_work, prev_s, prev_e, sel_aut, sel_cat, has_aut, has_cat)
...
if has_editorial:
    ...
    if not _chan_src.empty and "sessionDefaultChannelGroup" in _chan_src.columns:
        ca = (_chan_src.groupby("sessionDefaultChannelGroup", as_index=False).agg(U=("activeUsers","sum"), V=("screenPageViews","sum"), S=("sessions","sum")).sort_values("U", ascending=False))
        ...
        with cc2:
            ...
            st.dataframe(styled.style.format({"Usuarios":"{:,.0f}","Vistas":"{:,.0f}","Sesiones":"{:,.0f}"}), use_container_width=True, hide_index=True, height=260)
        if has_chan:
            sel_row = ca[ca["sessionDefaultChannelGroup"] == sel_chan]
            if not sel_row.empty:
                st.caption(f"📡 Canal **{sel_chan}**: {fmt_number(int(sel_row['U'].iloc[0]))} usuarios · {fmt_number(int(sel_row['V'].iloc[0]))} vistas · {fmt_number(int(sel_row['S'].iloc[0]))} sesiones")
    else:
...
if has_editorial:
    ...
    if not _city_src.empty and "city" in _city_src.columns:
        cv = (_city_src[_city_src["city"] != "(not set)"].groupby("city", as_index=False).agg(U=("activeUsers","sum")).sort_values("U", ascending=False).head(20))
...
if has_editorial:
    ...
    with pc1:
        f4 = px.choropleth(cv2, locations="country", locationmode="country names", color="U", color_continuous_scale=["#0a0a20","#6366f1","#06b6d4","#10b981"])
        f4.update_layout(geo=dict(bgcolor=PBG, showframe=False, landcolor="#10102a", showocean=True, oceancolor="#080814", showcoastlines=True, coastlinecolor="#1a1a3a"))
        _fig(f4, 340)
        st.plotly_chart(f4, use_container_width=True)
    with pc2:
        st.dataframe(cv2.head(25).rename(columns={"country":"País","U":"Usuarios"}).style.format({"Usuarios":"{:,.0f}"}), use_container_width=True, hide_index=True, height=320)
else:
...
with n1:
    ...
    if not urls.empty and "pagePath" in urls.columns:
        ...
        if not prod_work.empty and "url" in prod_work.columns and "post_author_name" in prod_work.columns:
            p2a = {}
            for u, a in zip(prod_work["url"], prod_work["post_author_name"]):
                pu = _norm_path(u)
                if pu:
                    p2a[pu] = a if _clean_text(a) else "Sin autor"
...
if not prod.empty and "categories" in prod.columns and "ga4_views" in prod.columns:
    sp = prod.copy()
    sp["cat"] = sp["categories"].fillna("Sin cat").apply(lambda x: ", ".join(_split_cats(x)) if has_cat else (_split_cats(x)[0] if _split_cats(x) else "Sin cat"))
    if "post_id" not in sp.columns:
        sp["post_id"] = range(1, len(sp) + 1)
    if "ga4_users" not in sp.columns:
        sp["ga4_users"] = 0
    ...
    with sc2:
        st.dataframe(sa.rename(columns={"cat":"Sección","U":"Usuarios","V":"Vistas","N":"Notas"}).style.format({"Usuarios":"{:,.0f}","Vistas":"{:,.0f}","Notas":"{:,.0f}"}), use_container_width=True, hide_index=True, height=300)
else:
...
if not prod.empty and "post_date" in prod.columns:
    tp = prod.copy()
    if "post_id" not in tp.columns:
        tp["post_id"] = range(1, len(tp) + 1)
...
if not prod.empty and "is_ia" in prod.columns:
    ...
    if not ia.empty:
        ...
        st.dataframe(ia[cols_].rename(columns={"post_title":"Título","post_author_name":"Autor","post_date":"Fecha","ga4_views":"Vistas","ga4_users":"Usuarios","match_method":"Origen"}).style.format({"Vistas":"{:,.0f}","Usuarios":"{:,.0f}"}), use_container_width=True, hide_index=True)
    else:
