...
# ── Detectar columnas YouTube ─────────────────────────────────────────────────
yt_date_col = next((c for c in [
    "Hora de publicación del vídeo",
...
yt_views_col = next((c for c in [
    "Visualizaciones",
    "Views",
    "Vistas"
] if not yt_gr_raw.empty and c in yt_gr_raw.columns), None)

yt_watch_col = next((c for c in [
    "Tiempo de visualización (horas)",
    "Tiempo de visualización",
    "Watch time (hours)"
] if not yt_gr_raw.empty and c in yt_gr_raw.columns), None)

yt_impr_col = next((c for c in [
    "Impresiones",
    "Impressions"
] if not yt_gr_raw.empty and c in yt_gr_raw.columns), None)

yt_ctr_col = next((c for c in [
...
with fc1:
    start = st.date_input("📅 Desde", max_d - timedelta(days=90), min_value=min_d, max_value=max_d, key="gs")
with fc2:
    end = st.date_input("📅 Hasta", max_d, min_value=min_d, max_value=max_d, key="ads_e")
with fc3:
...
rev_tot_p = (
    _s(as_p, "Estimated earnings (USD)")
    + _s(mg_p, "Revenue")
    + _s(gd_p, "AD_SERVER_CPM_AND_CPC_REVENUE")
    + rev_yt_p
)
...
impr_yt  = int(_s(yt_gr, yt_impr_col)) if yt_impr_col else 0
...
ctr_yt = yt_gr[yt_ctr_col].mean() if yt_ctr_col and not yt_gr.empty else 0
watch_yt = _s(yt_gr, yt_watch_col) if yt_watch_col else 0
...
if "AdSense" in plataformas and not as_f.empty:
    rows.append({"Plataforma": "AdSense", "Revenue": rev_as, "Impresiones": impr_as, "Clicks": cl_as, "CPM": _s(as_f, "Impression RPM (USD)")})
if "MGID" in plataformas and not mg_f.empty:
    rows.append({"Plataforma": "MGID", "Revenue": rev_mg, "Impresiones": impr_mg, "Clicks": cl_mg, "CPM": _s(mg_f, "Ad RPM")})
if "Ad Manager" in plataformas and not gd_f.empty:
    rows.append({"Plataforma": "Ad Manager", "Revenue": rev_gam, "Impresiones": impr_gam, "Clicks": cl_gam, "CPM": _s(gd_f, "AD_SERVER_WITHOUT_CPD_AVERAGE_ECPM")})
if "YouTube" in plataformas and not yt_gr_raw.empty:
    rows.append({"Plataforma": "YouTube", "Revenue": rev_yt, "Impresiones": impr_yt, "Clicks": 0, "CPM": 0, "CTR": ctr_yt, "Watch Time (h)": watch_yt})
...
if rows:
    ...
    if not r_df2.empty:
        ...
        with g1:
            fb = px.bar(
                r_df2, x="Plataforma", y="Revenue", text="Revenue",
        ...
        with g2:
            fp = px.pie(
                r_df2, names="Plataforma", values="Revenue",
...
for df_, dcol, plat, col_rev in [
    ...
    if not df_.empty and dcol in df_.columns and col_rev in df_.columns:
        t = df_.copy()
        t["mes"] = t[dcol].dt.to_period("M").astype(str)
        tmp = t.groupby("mes")[col_rev].sum().reset_index()
        tmp.columns = ["mes", "Revenue"]
        tmp["Plataforma"] = plat
        frames.append(tmp)
...
if "YouTube" in plataformas and not yt_gr.empty and yt_date_col and yt_rev_col:
    ...
    tmp.columns = ["mes", "Revenue"]
    tmp["Plataforma"] = "YouTube"
    frames.append(tmp)

if frames:
    comb = pd.concat(frames, ignore_index=True)
    fe = px.bar(
        comb, x="mes", y="Revenue", color="Plataforma", barmode="stack",
...
if "Ad Manager" in plataformas and not gd_f.empty and "DATE" in gd_f.columns:
    ...
    gm = gd_f.copy()
    gm["mes"] = gm["DATE"].dt.to_period("M").astype(str)
    ag = {}
    if "AD_SERVER_IMPRESSIONS" in gm.columns:
        ag["AD_SERVER_IMPRESSIONS"] = "sum"
    if "TOTAL_LINE_ITEM_LEVEL_IMPRESSIONS" in gm.columns:
        ag["TOTAL_LINE_ITEM_LEVEL_IMPRESSIONS"] = "sum"
    if ag:
        ...
        if "AD_SERVER_IMPRESSIONS" in gam_m.columns:
                ...
                x=gam_m["mes"], y=gam_m["AD_SERVER_IMPRESSIONS"],
                name="Servidas", mode="lines+markers",
                line=dict(color=C[0], width=3), marker=dict(size=7)
            ))
        if "Sin rellenar" in gam_m.columns:
            fi.add_trace(go.Scatter(
                x=gam_m["mes"], y=gam_m["Sin rellenar"],
                name="Sin rellenar", mode="lines+markers",
                line=dict(color=C[4], width=2.5, dash="dash"), marker=dict(size=6)
            ))
...
if "Ad Manager" in plataformas:
    ...
    if not gam_fmt.empty and "CREATIVE_SIZE" in gam_fmt.columns:
        ...
        if "Impresiones" in fd2.columns:
            fd2 = fd2.sort_values("Impresiones", ascending=False)
        ...
        if "Impresiones" in fd2.columns:
            ff = px.bar(
                fd2.head(10), x="Impresiones", y="Formato", orientation="h",
...
if "AdSense" in plataformas and not as_f.empty and "Date" in as_f.columns:
    ...
    ac = as_f.copy()
    ac["mes"] = ac["Date"].dt.to_period("M").astype(str)
    ag2 = {}
    if "Estimated earnings (USD)" in ac.columns:
        ag2["Estimated earnings (USD)"] = "sum"
    if "Impressions" in ac.columns:
        ag2["Impressions"] = "sum"
    if "Clicks" in ac.columns:
        ag2["Clicks"] = "sum"
    if ag2:
...
if "MGID" in plataformas and not mg_f.empty:
    ...
    cm = {
        ...
        "Ad RPM": "RPM",
        "Ad vRPM": "vRPM",
        "Views with visibility": "Vistas Visibles"
    }
    s2 = {k: v for k, v in cm.items() if k in mg_f.columns}
    mg_s = mg_f[list(s2.keys())].rename(columns=s2)
    if "Fecha" in mg_s.columns:
        mg_s = mg_s.sort_values("Fecha", ascending=False)
    nf2 = {}
    if "Revenue" in mg_s.columns:
        nf2["Revenue"] = "${:,.3f}"
    if "RPM" in mg_s.columns:
        nf2["RPM"] = "${:,.3f}"
    if "vRPM" in mg_s.columns:
        nf2["vRPM"] = "${:,.3f}"
...
if "YouTube" in plataformas and not yt_gr_raw.empty and yt_date_col:
    ...
    if src_yt.empty:
        ...
        if yt_views_col and yt_views_col in ytc.columns:
            ag_yt[yt_views_col] = "sum"
        if yt_rev_col and yt_rev_col in ytc.columns:
            ag_yt[yt_rev_col] = "sum"
        if ag_yt:
            yt_m = ytc.groupby("mes").agg(ag_yt).reset_index()
            fyt = go.Figure()
            if yt_views_col and yt_views_col in yt_m.columns:
                fyt.add_trace(go.Bar(
                    x=yt_m["mes"], y=yt_m[yt_views_col],
                    name="Visualizaciones", marker_color=C[4], opacity=0.7
                ))
            if yt_rev_col and yt_rev_col in yt_m.columns:
                fyt.add_trace(go.Scatter(
                    x=yt_m["mes"], y=yt_m[yt_rev_col],
...
if "Ad Manager" in plataformas:
    ...
    if not gd.empty and "DEVICE_CATEGORY_NAME" in gd.columns:
        ...
        with gd2:
            ...
            nf3 = {}
            if "Impresiones" in gd_s.columns:
                nf3["Impresiones"] = "{:,.0f}"
            if "Revenue" in gd_s.columns:
                nf3["Revenue"] = "${:,.2f}"
            if "eCPM" in gd_s.columns:
                nf3["eCPM"] = "${:,.2f}"
