...
st.markdown('<div class="page-subtitle">AdSense · MGID · Ad Manager · YouTube Revenue · MOW</div>', unsafe_allow_html=True)
...
with fc3:
    plataformas = st.multiselect(
        "💳 Plataforma",
        ["AdSense", "MGID", "Ad Manager", "YouTube", "MOW"],
        default=["AdSense", "MGID", "Ad Manager", "YouTube", "MOW"]
    )
...
rev_mow = mow_rev if "MOW" in plataformas else 0
rev_yt_p = 0
rev_mow_p = 0

rev_tot = rev_as + rev_mg + rev_gam + rev_yt + rev_mow
rev_tot_p = (
...
if "MOW" in plataformas:
    rows.append({"Plataforma": "MOW", "Revenue": rev_mow, "Impresiones": 0, "Clicks": 0, "CPM": 0})
...
if "YouTube" in plataformas and not yt_gr_raw.empty and yt_date_col:
    ...
    if src_yt.empty:
        ...
        if not yt_tb_raw.empty:
            ...
            cols_show = [c for c in ["Título del vídeo", "Visualizaciones", "Ingresos estimados (USD)", "Impresiones", "Porcentaje de clics de las impresiones (%)"] if c in yt_tb_raw.columns]
            top_v = yt_tb_raw[cols_show].sort_values("Visualizaciones", ascending=False).head(15)
            nf_yt = {}
            if "Visualizaciones" in top_v.columns:
                nf_yt["Visualizaciones"] = "{:,.0f}"
            if "Ingresos estimados (USD)" in top_v.columns:
                nf_yt["Ingresos estimados (USD)"] = "${:,.2f}"
            if "Impresiones" in top_v.columns:
                nf_yt["Impresiones"] = "{:,.0f}"
