...
def _ensure_dt(df: pd.DataFrame, col: str) -> pd.DataFrame:
    """Garantiza que la columna sea datetime (no string)."""
...
# Garantizar datetime en fecha_post de CSVs sociales
ig_raw  = _ensure_dt(ig_raw,  "fecha_post")
igs_raw = _ensure_dt(igs_raw, "fecha_post")
fb_raw  = _ensure_dt(fb_raw,  "fecha_post")

# YouTube — "grafico" y "tabla" apuntan al mismo df con columna "Fecha"
yt_g = _ensure_dt(yt.get("grafico", pd.DataFrame()), "Fecha")
yt_t = yt_g   # alias — mismo objeto, se filtra por fecha igual que el resto
...
with st.expander("🔬 Debug — archivos cargados", expanded=False):
    ...
    if not yt_g.empty:
        mn_yt = yt_g["Fecha"].min().date() if yt_g["Fecha"].notna().any() else "?"
        mx_yt = yt_g["Fecha"].max().date() if yt_g["Fecha"].notna().any() else "?"
        st.success(f"✅ YouTube: {len(yt_g)} filas | {mn_yt} → {mx_yt}")
        st.caption(f"Columnas: {', '.join(yt_g.columns[:10])}")
...
if not yt_g.empty and "Fecha" in yt_g.columns:
    all_d += yt_g["Fecha"].dropna().tolist()
...
# YouTube filtrado por fecha — mismo mecanismo que los CSVs
yt_gf = filter_by_date(yt_g, "Fecha", start, end) if not yt_g.empty and "Fecha" in yt_g.columns else pd.DataFrame()
...
yt_gp  = filter_by_date(yt_g, "Fecha", ps, pe) if not yt_g.empty and "Fecha" in yt_g.columns else pd.DataFrame()
...
# ✅ YouTube: siempre usar yt_gf (filtrado) — nunca yt_t/yt_g completos
yt_sf = yt_gf if "YouTube"           in redes else pd.DataFrame()
...
fb_reach  = _s(fb_s, "Alcance")
# Buscar columnas con o sin tilde
fb_v3     = _s(fb_s, "Visualizaciones de vídeo de 3 segundos",
...
# ✅ YouTube — SOLO datos filtrados por fecha (yt_sf)
yt_plays = _s(yt_sf, "Visualizaciones")
yt_impr  = _s(yt_sf, "Impresiones")
yt_subs  = _s(yt_sf, "Suscriptores")
yt_rc    = _fc(yt_sf, "Ingresos estimados (USD)", "Ingresos estimados")
yt_rev   = safe_sum(yt_sf, yt_rc) if yt_rc and not yt_sf.empty else 0.0

# ── Previos ───────────────────────────────────────────────────────────────────
ig_p_v   = _s(ig_p,  "Visualizaciones"); ig_p_r   = _s(ig_p,  "Alcance")
igs_p_v  = _s(igs_p, "Visualizaciones"); igs_p_r  = _s(igs_p, "Alcance")
fb_p_r   = _s(fb_p,  "Alcance")
fb_p_v3  = _s(fb_p,  "Visualizaciones de vídeo de 3 segundos",
                       "Visualizaciones de video de 3 segundos")
# ✅ YouTube previo también filtrado (yt_gp)
yt_p_v   = _s(yt_gp, "Visualizaciones")
yt_p_rc  = _fc(yt_gp, "Ingresos estimados (USD)", "Ingresos estimados")
yt_p_rev = safe_sum(yt_gp, yt_p_rc) if yt_p_rc and not yt_gp.empty else 0.0
...
m1.metric("👁 Impresiones/Plays", fmt_number(total_v),  _delta_str(total_v,  ig_p_v+igs_p_v+fb_p_v3+yt_p_v))
m2.metric("📢 Alcance Total",     fmt_number(total_r),  _delta_str(total_r,  ig_p_r+igs_p_r+fb_p_r))
m3.metric("▶️ YouTube Plays",     fmt_number(yt_plays), _delta_str(yt_plays, yt_p_v))
...
if "YouTube"           in redes and not yt_sf.empty:
    # ✅ Usar yt_sf (filtrado) para el resumen
    rows.append({"Red":"▶️ YouTube",     "Posts":len(yt_sf), "Impresiones":yt_plays, "Alcance":yt_impr,
                 "Me gusta":0, "Comentarios":0, "Compartidos":0, "Guardados":0})
if rows:
...
if "Instagram Posts" in redes:
    ...
    if ig_s.empty:
        ...
        tc = _fc(ig_s, "Tipo de publicación", "Tipo de publicacion")
...
if "Facebook" in redes:
    ...
    if fb_s.empty:
        ...
        v3_col = _fc(fb2, "Visualizaciones de vídeo de 3 segundos",
                          "Visualizaciones de video de 3 segundos")
        cols_fb = [c for c in ["Alcance", v3_col] if c and c in fb2.columns]
        if cols_fb:
            ev3 = fb2.groupby("mes")[cols_fb].sum().reset_index()
            fig_f = go.Figure()
            if v3_col and v3_col in ev3.columns:
                fig_f.add_trace(go.Bar(x=ev3["mes"], y=ev3[v3_col],
                    name="Views 3s", marker_color="#ef4444", opacity=0.4, yaxis="y2"))
            if "Alcance" in ev3.columns:
        ...
        if tc_fb and sr_c:
            ...
            show_r = [c for c in [tc_fb, "Hora de publicación", "Alcance", v3_col, sr_c]
                      if c and c in top_r.columns]
...
if "YouTube" in redes:
    ...
    if yt_g.empty:
        ...
        # ✅ TODAS las métricas y visualizaciones usan yt_sf (filtrado por fecha)
        y1,y2,y3,y4 = st.columns(4)
        y1.metric("▶️ Visualizaciones", fmt_number(yt_plays),  _delta_str(yt_plays, yt_p_v))
        y2.metric("📢 Impresiones",     fmt_number(yt_impr))
        y3.metric("🔔 Suscriptores",    fmt_number(yt_subs))
        y4.metric("💰 Revenue",         f"${yt_rev:.2f}",      _delta_str(yt_rev,   yt_p_rev))

        # Evolución mensual — con datos filtrados
        ytc = yt_sf.copy()
        ytc["mes"] = pd.to_datetime(ytc["Fecha"], errors="coerce").dt.to_period("M").astype(str)
        evo_cols = [c for c in ["Visualizaciones", "Ingresos estimados (USD)"] if c in ytc.columns]
        if evo_cols:
            yt_m = ytc.groupby("mes")[evo_cols].sum().reset_index()
            fyt  = go.Figure()
            if "Visualizaciones" in yt_m.columns:
                fyt.add_trace(go.Bar(x=yt_m["mes"], y=yt_m["Visualizaciones"],
                    name="Visualizaciones", marker_color="#ef4444", opacity=0.7))
            if "Ingresos estimados (USD)" in yt_m.columns:
                fyt.add_trace(go.Scatter(x=yt_m["mes"], y=yt_m["Ingresos estimados (USD)"],
            ...
            _fig(fyt, 280); st.plotly_chart(fyt, use_container_width=True)

        # Top vídeos — filtrados por período
        tc_yt = _fc(yt_sf, "Título del vídeo", "Titulo del video", "Título")
        vc_yt = _fc(yt_sf, "Visualizaciones")
        if tc_yt and vc_yt:
            sh("🏆 Top Vídeos YouTube (período seleccionado)")
            tc_show = [c for c in [tc_yt, "Hora de publicación del vídeo", "Duración", vc_yt,
                                   ...
                                   "Suscriptores", "Ingresos estimados (USD)"] if c in yt_sf.columns]
            st.dataframe(
                yt_sf.sort_values(vc_yt, ascending=False).head(30)[tc_show]
                .style.format({c:"{:,.0f}" for c in yt_sf.select_dtypes("number").columns}),
                use_container_width=True, hide_index=True)
