...
def _fc(df, *cols):
    for c in cols:
        if not df.empty and c in df.columns: return c
    return None
...
# Aplicar filtros — fecha real está en 'fecha_post'
...
if "Instagram Posts"   in redes and not ig_s.empty:
    rows.append({"Red":"📸 IG Posts","Posts":ig_n,"Impresiones":ig_views,"Alcance":ig_reach,"Me gusta":ig_likes,"Comentarios":ig_comm,"Compartidos":ig_share,"Guardados":ig_save})
if "Instagram Stories" in redes and not igs_s.empty:
    rows.append({"Red":"💬 IG Stories","Posts":igs_n,"Impresiones":igs_views,"Alcance":igs_reach,"Me gusta":_s(igs_s,"Me gusta"),"Comentarios":igs_resp,"Compartidos":igs_click,"Guardados":0})
if "Facebook"          in redes and not fb_s.empty:
    rows.append({"Red":"👥 Facebook","Posts":fb_n,"Impresiones":fb_v3,"Alcance":fb_reach,"Me gusta":fb_react,"Comentarios":fb_comm,"Compartidos":fb_share,"Guardados":0})
if "YouTube"           in redes and not yt_t.empty:
    rows.append({"Red":"▶️ YouTube","Posts":int(_s(yt_t,"Visualizaciones")),"Impresiones":yt_plays,"Alcance":0,"Me gusta":0,"Comentarios":0,"Compartidos":0,"Guardados":0})
if rows:
...
if "Instagram Posts" in redes:
    ...
    if ig_s.empty:
        ...
        if tc and "Alcance" in ig_s.columns:
            ...
            with tt1:
                ft=px.pie(tipo_agg,names=tc,values="Alcance",color_discrete_sequence=C,hole=0.5,title="Por formato")
            ...
            with tt2:
                ...
                st.dataframe(tipo_agg.style.format({c:"{:,.0f}" for c in nc_}),use_container_width=True,hide_index=True)
        ...
        if cols_evo:
            ...
            if "Visualizaciones" in evo.columns:
                fig2.add_trace(go.Bar(x=evo["sem"],y=evo["Visualizaciones"],name="Views",marker_color="#06b6d4",opacity=0.4,yaxis="y2"))
            fig2.add_trace(go.Scatter(x=evo["sem"],y=evo["Alcance"],name="Alcance",mode="lines+markers",line=dict(color="#8b5cf6",width=2.5),marker=dict(size=7)))
            fig2.update_layout(yaxis2=dict(overlaying="y",side="right",showgrid=False),barmode="overlay",legend=dict(orientation="h",y=1.12))
        ...
        with st.expander("📋 Todos los posts de Instagram (por Alcance)",expanded=False):
            sc_=[c for c in ["Descripción","Tipo de publicación","Hora de publicación","Alcance","Visualizaciones","Me gusta","Comentarios","Veces que se ha compartido","Veces guardado","Seguidores","Enlace permanente"] if c in ig_s.columns]
            s_c=_fc(ig_s,"Alcance","Visualizaciones")
            if sc_ and s_c:
                st.dataframe(ig_s[sc_].sort_values(s_c,ascending=False).style.format({c:"{:,.0f}" for c in ig_s.select_dtypes("number").columns}),use_container_width=True,hide_index=True)
...
if "Instagram Stories" in redes:
    ...
    if igs_s.empty:
        ...
        if cols_ev:
            ...
            fig_s=px.bar(ev2,x="sem",y="Alcance",color_discrete_sequence=[C[6]],text="Alcance",title="Alcance semanal Stories")
        ...
        with st.expander("📋 Todas las stories",expanded=False):
            sc2=[c for c in ["Descripción","Tipo de publicación","Hora de publicación","Visualizaciones","Alcance","Me gusta","Respuestas","Clics en el enlace","Seguidores"] if c in igs_s.columns]
            s_c2=_fc(igs_s,"Alcance","Visualizaciones")
            if sc2 and s_c2: st.dataframe(igs_s[sc2].sort_values(s_c2,ascending=False),use_container_width=True,hide_index=True)
    ...
    if fb_s.empty:
        ...
        if cols_fb:
            ...
            if "Visualizaciones de vídeo de 3 segundos" in ev3.columns:
                fig_f.add_trace(go.Bar(x=ev3["mes"],y=ev3["Visualizaciones de vídeo de 3 segundos"],name="Views 3s",marker_color="#ef4444",opacity=0.4,yaxis="y2"))
            fig_f.add_trace(go.Scatter(x=ev3["mes"],y=ev3["Alcance"],name="Alcance",mode="lines+markers",line=dict(color="#6366f1",width=2.5),marker=dict(size=7)))
            fig_f.update_layout(yaxis2=dict(overlaying="y",side="right",showgrid=False),barmode="overlay",legend=dict(orientation="h",y=1.12))
        ...
        if sr_c and tc_:
            ...
            show_r=[c for c in [tc_,"Hora de publicación","Alcance","Visualizaciones de vídeo de 3 segundos",sr_c] if c in top_r.columns]
            sh("⏱ Vídeos con mayor retención")
            st.dataframe(top_r[show_r].style.format({c:"{:,.1f}" for c in top_r.select_dtypes("float").columns}),use_container_width=True,hide_index=True)

        with st.expander("📋 Todos los vídeos de Facebook",expanded=False):
            sc3=[c for c in ["Título","Hora de publicación","Alcance","Visualizaciones de vídeo de 3 segundos","Visualizaciones de vídeo de 1 minuto","Reacciones","Comentarios","Veces que se ha compartido","Segundos reproducidos de media"] if c in fb_s.columns]
            s_c3=_fc(fb_s,"Alcance")
            if sc3 and s_c3: st.dataframe(fb_s[sc3].sort_values(s_c3,ascending=False).style.format({c:"{:,.1f}" for c in fb_s.select_dtypes("float").columns}),use_container_width=True,hide_index=True)
...
if "YouTube" in redes:
    ...
    if yt_t.empty and yt_gf.empty:
        ...
        if not yt_gf.empty and "Fecha" in yt_gf.columns and "Visualizaciones" in yt_gf.columns:
            ytc=yt_gf.copy(); ytc["mes"]=pd.to_datetime(ytc["Fecha"],errors="coerce").dt.to_period("M").astype(str)
            yt_e=ytc.groupby("mes")["Visualizaciones"].sum().reset_index()
            fy=px.bar(yt_e,x="mes",y="Visualizaciones",color="Visualizaciones",color_continuous_scale=["#1a0808","#ef4444"],text="Visualizaciones",title="Visualizaciones YouTube por Mes")
            fy.update_traces(texttemplate="%{text:,.0f}",textposition="outside",textfont_size=9)
            fy.update_layout(coloraxis_showscale=False); _fig(fy,260); st.plotly_chart(fy,use_container_width=True)

        tc_yt=_fc(yt_t,"Título del vídeo","Título"); vc_yt=_fc(yt_t,"Visualizaciones")
        if tc_yt and vc_yt and not yt_t.empty:
            sh("🏆 Top Vídeos YouTube")
            tc_show=[c for c in [tc_yt,"Hora de publicación del vídeo","Duración",vc_yt,"Tiempo de visualización (horas)","Impresiones","Porcentaje de clics de las impresiones (%)","Suscriptores","Ingresos estimados (USD)"] if c in yt_t.columns]
            st.dataframe(yt_t.sort_values(vc_yt,ascending=False).head(30)[tc_show].style.format({c:"{:,.0f}" for c in yt_t.select_dtypes("number").columns}),use_container_width=True,hide_index=True)
